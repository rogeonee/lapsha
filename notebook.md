...
- **2026-06-10 — `expo-image` `sf:` sources are iOS-only**; Android's Glide rejects the scheme and renders nothing (silent, error only in logcat). Shared components must use `components/ui/icons.tsx` (SF on iOS, Material SVG on Android). HeroUI `Select` also requires the `presentation` prop to match between root and `Select.Content` or it throws.
