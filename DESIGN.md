...
- **Primary:** Ink Primary (#18181B) fill, near-white (#FAFAFA) medium text — deliberately *not* amber; amber is for accents, primary actions are calm ink
...
### Avatar
A Cream Swirl circle in three sizes: 40px on timeline rows, 48px on list cards, 72px as the centered identity header on the add-person and person screens. Content priority: the person's photo (center-cropped to the circle) → Broth semibold initial → Broth person glyph (no name yet). Photos inherit the initial-avatar geometry exactly — no rings, borders, or badges. The 72px header pairs with a Broth "Add photo / Change photo" caption beneath it (Contacts-style); that caption is the only photo affordance, and tapping an existing photo offers choose/remove through the platform's own menu.
