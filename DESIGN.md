...
### Avatar
A Cream Swirl circle in four sizes: 40px on timeline rows, 48px on list cards, 72px in the add-person preview, and 96px as the centered identity header on the person screen. Content priority: the person's photo (center-cropped to the circle) → Broth semibold initial → Broth person glyph (no name yet). Photos inherit the initial-avatar geometry exactly — no rings, borders, or badges. The 96px person header pairs with a separate Broth "Add photo / Change photo" action beneath it (Contacts-style). When a photo exists, tapping or pulling down the compact circle expands that same square crop to full screen width, behind the transparent native header and status bar, and pushes the person's details down; tapping or dragging upward collapses it. The edit caption disappears as soon as the growing photo reaches it. Initials and the person glyph never expand. Photo viewing and photo editing remain separate actions.
