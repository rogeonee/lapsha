...
- [ ] **B02 · P1 — Preserve February 29 when editing an unknown-year date.** Executing the actual helpers in 2026 produced `0001-02-29 → editor March 1, 2026 → 0001-03-01`. Merely editing the label and saving changes the saved birthday. Use a leap-safe representation for unknown years; separately choose the anniversary policy for non-leap years without changing the stored month/day. Strengthen validation of known-year dates: the current schema also accepts `2026-02-30` and `2026-04-31`. Preserve the intentional `0001-02-29` sentinel when tightening validation. Evidence: [date conversion](/Users/rogeonee/dev/projects/lapsha/src/lib/dates.ts:23), [date schema](/Users/rogeonee/dev/projects/lapsha/src/api/dates/date-schema.ts).

- [ ] **B03 · P1 — Initialize Android date pickers using UTC calendar components.** Both Android picker callers pass a local date through `toISOString()`. In `Asia/Tokyo`, stored `1990-04-02` becomes `1990-04-01T15:00:00.000Z`; Material's UTC date selection consequently points at April 1. Confirming that selection can change the saved day. The return conversion already uses UTC getters; fix the input conversion in both entry and add-person pickers too. Confirmed on the physical Pixel 6 in `Asia/Tokyo`: a saved September 9 birthday opened with September 8 selected in the native picker. Cancelled without saving; restored `America/Edmonton` and automatic timezone afterward. Evidence: [entry picker](/Users/rogeonee/dev/projects/lapsha/src/components/entry/entry-sheet.android.tsx:323), [birthday picker](/Users/rogeonee/dev/projects/lapsha/src/components/person/birthday-date-row.android.tsx:45), [Material picker UTC contract](https://developer.android.com/reference/kotlin/androidx/compose/material3/rememberDatePickerState.composable).

- [ ] **B04 · P1 — Keep Android Fact/Date drafts consistent with the visible fields.** Confirmed on the Pixel 6: type `Strawberry coffee` as a fact, switch to Date, append ` date`, switch back to Fact, then Save. The Fact field visibly contains `Strawberry coffee date`, but the saved value is `Strawberry coffee`. The date label also initially inherits the fact text while Save remains disabled. The native uncontrolled input is reused across modes while the hook keeps separate values. Preserve each draft and bind/remount the correct input so the displayed value always matches the saved state. Evidence: [Android inputs](/Users/rogeonee/dev/projects/lapsha/src/components/entry/entry-sheet.android.tsx:264), [shared form state](/Users/rogeonee/dev/projects/lapsha/src/components/entry/use-entry-form.ts:59).

- [ ] **B05 · P1 for real-data use — Decide and verify backup/recovery behavior.** There is no export/import UI. More concretely, the Android release APK inherits `expo-secure-store` rules that include only the `sharedpref` domain in both cloud backup and device transfer. Lapsha's SQLite database and avatar files are excluded. Confirmed by decoding the APK's compiled XML, not just reading `allowBackup=true`. Provide appropriate rules and test restoring people, facts, dates, and photos together, or deliberately keep this first beta disposable and tell testers clearly that recovery is unavailable. A backend is not required. If enabling OS cloud backups, make the privacy copy describe them accurately and account for Android's 25 MB backup quota, especially with photos. Evidence: [app plugins](/Users/rogeonee/dev/projects/lapsha/app.json), [backup rules](/Users/rogeonee/dev/projects/lapsha/node_modules/expo-secure-store/android/src/main/res/xml/secure_store_data_extraction_rules.xml), [Android backup inclusion rules](https://developer.android.com/identity/data/autobackup#IncludeExclude).
...
- [ ] Photo pick, crop, cancel, change, remove, expand/collapse, and missing-file fallback. Verify photos survive app updates and the chosen restore path.
...
- [ ] Install candidate N, create records/photos/preferences, update to N+1 without uninstalling, then verify all data. Exercise database v1/v2 migration fixtures and the actual OS backup/restore path separately.
